...
export function HrAttendanceClientPage({ currentUser, allUsers }: Props) {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-slate-100 font-sans">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-2.5 flex items-center justify-between shadow-sm z-10 gap-4 overflow-hidden">
                {/* Título & Abas Pills Estilo WaSeller (Scrollavel livre) */}
                <div className="flex items-center gap-4 overflow-x-auto min-w-0 flex-1 py-1 no-scrollbar">
                    <div className="flex items-center gap-2 flex-shrink-0 mr-2">
                        ...
                        <div>
                            ...
                            <span className="text-[10px] text-emerald-600 font-semibold whitespace-nowrap">WhatsApp RH ao Vivo</span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 overflow-x-auto pr-2 no-scrollbar">
                        {stages.map((stg) => {
                            ...
                            return (
                                <div
                                    key={stg.id}
                                    className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-700 border border-slate-200 flex-shrink-0 shadow-2xs whitespace-nowrap"
                                >
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {currentUser?.role === "ADMIN" && (
                        <Button
                            variant="outline"
                            size="sm"
                            className="h-8 text-xs gap-1 border-slate-300 text-slate-700 flex-shrink-0"
