...
export async function POST(request: Request) {
    try {
        ...
        const docId = body?.event?.data?.object?.id;

        if (!docId) {
            return NextResponse.json({ message: "No document ID found." }, { status: 400 });
        }

        // Check if the event indicates document was signed or completed
        if (eventType === "document.signed" || eventType === "document.finished" || eventType === "document.completed") {
